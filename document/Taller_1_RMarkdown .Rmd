...
De acuerdo a la forma específica funcional establecida para el perfil de edad-salario, presentada a continuación: 
$$
...
```{ r, echo=FALSE}
regresion_salario_edad <- lm (log_salarioreal~age +age_cuadradofinal,geih_filtered)
...
eta_fn(geih_filtered,1:nrow(geih_filtered))
boot(geih_filtered, eta_fn, R = 1000)

##Boot 2
eta_fn2<-function(data,index){
  coef(lm(log_salarioreal~age +age_cuadradofinal, data = data, subset = index))[3]}

eta_fn2(geih_filtered,1:nrow(geih_filtered))
boot(geih_filtered, eta_fn2, R = 1000)
...
intervalodeconfianza_p3 <- data.frame(Variables = c("edad", "edad^2"),
                                      `Intervalos de confianza` = c("(0.049, 0.0668)", "(-0.0007, -0.0005)"))
